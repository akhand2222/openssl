...
The lookup first consults the list of untrusted "intermediate" CA certificates
and if no match is found there
the remaining lookups are from the trusted certificates.
...
and the C<notBefore> and C<notAfter> dates in the certificate.
The B<-attime> flag may be used to specify a time other than "now."
The certificate signatures are also checked at this point
(except for the signature of the self-signed root certificate,
...
is checked to permit the respective signing operation.
...
which typically is a trusted self-signed root CA certificate.
This check is disabled by default because it does not add any security.
