...
the certificate chain could be built up using the untrusted certificates but the
(typically self-signed) root certificate could not be found in the trust store.
