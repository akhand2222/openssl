...
To enable termination of a chain by a certificate of an intermediate CA
or by a trusted self-issued but not self-signed certificate
the B<X509_V_FLAG_PARTIAL_CHAIN> flag may be set.
...
the self-signed root certificate could not be found in the trust store.
...
(nor self-issued in case the B<X509_V_FLAG_PARTIAL_CHAIN> flag is set).
